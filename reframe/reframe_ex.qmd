...
fits <- mtcars |> 
  group_by(cyl) |> 
  reframe(fit = list(fit_hp_mpg(cur_data()))) # cur_data() picks the grouped data frame

fits
```

We need to use `list` because the result of `fit_hp_mpg` is not a vector, but a data frame.

Now we have a data frame "fit" for each group of cars with the same number of cylinders. We can unpack the fits using `unnest`.

```{r}
fits <- mtcars |> 
  group_by(cyl) |> 
  reframe(fit = list(fit_hp_mpg(cur_data()))) |> 
  unnest(fit)
